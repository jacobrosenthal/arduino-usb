        ...
        async.whilst(
          ...
          function(pagedone) {
            ...
            async.series([
              ...
              function(cbdone){
                writeBytes = hex.slice(pageaddr, (hex.length > options.board.pageSize ? (pageaddr + options.board.pageSize) : hex.length - 1));
                cbdone();
              },
              function(cbdone){
                loadPage(tinyAVR, pageaddr, writeBytes, 2000, cbdone);  
              },
              function(cbdone){
                loadAddress(tinyAVR, useaddr, 2000, cbdone);
              },
              function(cbdone){
