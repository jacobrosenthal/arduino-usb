...
Board.prototype.bootload = function bootload(options, cb){
  ...
  return nodefn.bindCallback(when.promise(function(resolve, reject) {
    ...
    //make a generic logger and attach to options the tinyavr programmer
    function log(){
      self.log(['info', 'programming'], util.format.apply(this, arguments));
    }
...
Board.prototype.compile = function bootload(options, cb){
  ...
  .then(function (response) {
    ...
    self.log(['info', 'compile'], response.entity.log);

    var memory = {
      type: 'flash',
      data: ihex.parse(response.entity.hex).data
    };
